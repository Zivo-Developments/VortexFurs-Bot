...
export default class Verification {
    ...
    private async sendVerificationRequest() {
        const embed = new MessageEmbed()
            ...
            .setColor(this.client.config.color)
            .addField("Troll Meter", this.trollMeter.toString() || (0).toString(), true)
            .setThumbnail(this.user.displayAvatarURL({ dynamic: true }))
        ...
        if (this.trollReasons!.length > 0) {
            embed.setDescription(`\`\`\`Troll Meter Reasons:\n ${this.trollReasons!.join("\n")}\`\`\``);
        }
        for (let i = 0; i < this.questions.length; i++) {
