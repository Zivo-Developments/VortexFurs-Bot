...
export async function channelResolver(client: FuzzyClient, mention: string): Promise<Channel | null> {
	...
	const channel = channelRegex.test(mention) ? await client.channels.fetch(channelRegex.exec(mention)![1]).catch(() => null) : null;
	if (channel) return channel;
	return null;
}

export async function emojiResolver(client: FuzzyClient, mention: string): Promise<Emoji> {
...
export async function messageResolver(message: Message, snowflake: string): Promise<Message> {
	const msg = snowflakeRegex.test(snowflake) ? await message.channel.messages.fetch(snowflake).catch(() => null) : undefined;
	if (msg) return msg;
